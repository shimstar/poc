...
class bulletTest(DirectObject):
	def __init__(self):
		...
		base.camera = base.makeCamera(base.win) 
		...
		visNP = loader.loadModel('fighter')
		visNP.setShaderAuto()
		visNP.ls()
		...
		inputState.watchWithModifiers('glow', 'v')
		inputState.watchWithModifiers('shake', 'c')
	...
	def update(self,task):
		...
		if globalClock.getRealTime()-self.updateInput>0.1:
			...
			if inputState.isSet('explosion'):
				...
				self.forceGrav=10000000.0
			if inputState.isSet('shake'):
				#~ if self.shake==True:
					#~ self.shake=False
					#~ print "ooo"
				#~ else:
				self.shake=True
			if inputState.isSet('glow'):
				if self.glow==True:
					self.glow=False
				else:
					self.glow=True
		...
		if self.glow==True:
			...
			base.setBackgroundColor(1,0,0,1) # bug? 
		...
		if self.shake==True:
			#~ if self.returnToNormal==False:
			x=random.randint(-1,1)
			z=random.randint(-1,1)
			base.camera.setPos(base.camera,(x*5,0,z*5))
			print self.shake
				#~ self.returnToNormal=True
			#~ else:
				#~ self.returnToNormal=False
